...
;; -------------------------------------------------------
;; -------------------------------------------------------
;; renderFloorCPU -- main CPU floor raycasting (row-major, Lodev style)
;; For each row: precompute floorX/Y, step across columns.
;; Walls draw AFTER this, so floor covers entire bottom half.
;; -------------------------------------------------------
renderFloorCPU:
    php
    rep #$30
...
.INDEX 16
    ...
    ; Row-major: for each row, step floorX/Y across columns
    lda #$0000
    sta $40              ; row index
...
@FRRow:
    ...
    ; floorX_start = posX + fp_mul(rowDist, leftRayDirX)
    lda $8C
    tay
    lda $80
    jsr fp_mul_hw
    clc
    adc.l posX
    sta $90              ; floorX (will be stepped)

    ; floorY_start = posY + fp_mul(rowDist, leftRayDirY)
    lda $8C
    tay
    lda $82
    jsr fp_mul_hw
...
@FRCol:
    ; texU = (floorX >> 3) & 31, texV = (floorY >> 3) & 31
    lda $90
    lsr a
    lsr a
    lsr a
...
@CFCol:
    ; Set WMADD = columnstart[col] + 41 (floor starts at screen row 41)
    stx $44              ; save col
    rep #$20
.ACCU 16
    txa
...
.ACCU 8
    ...
    ; Y = BW-RAM start offset = col (row 0, column col)
    rep #$20
.ACCU 16
    lda $44
    and #$00FF
...
.ACCU 16
    tya
    clc
    adc #112             ; next row in row-major BW-RAM
    tay
    sep #$20
.ACCU 8
    dec $40
