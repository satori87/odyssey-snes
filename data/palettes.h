/* ============================================
 * Shared 16-color BGR555 palette
 * Auto-generated by tools/convert_textures_c.py
 * ============================================ */

#ifndef PALETTES_H
#define PALETTES_H

/* Colors 0-15: texture palette */
const u16 shared_palette[16] = {
    0x0000, 0x1D07, 0x14A5, 0x2549, 0x150C, 0x0066, 0x0CEA, 0x08A9, 0x1484, 0x25B2, 0x1D09, 0x318C, 0x0087, 0x1CE7, 0x14A5, 0x35AD
};
/* Colors 16-17: ceiling and floor solid */
const u16 extra_palette[2] = {
    0x1084,  /* 16: dark gray ceiling (#222222) */
    0x1CE7   /* 17: medium gray floor (#383838) */
};

/* Colors 32-47: darkened texture palette (for ceiling) */
const u16 dark_palette[16] = {
    0x0000, 0x0C83, 0x0842, 0x10A4, 0x0886, 0x0023, 0x0465, 0x0444, 0x0842, 0x10C9, 0x0C84, 0x18C6, 0x0043, 0x0C63, 0x0842, 0x18C6
};

#endif /* PALETTES_H */
